...
public abstract class DequeTests {
    ...
    void removeAfterAddToOppositeEndsReturnsCorrectItem() {
        ...
        //if b equals output, else fail test
    }
    ...
    class RuntimeExperiments {
        ...
        void addLastGetMiddle() {
            for (int size = STEP; size <= MAX_SIZE; size += STEP) {
                ...
                double totalAddFirstTime = 0.0;
                double totalRemoveFirstTime = 0.0;
                ...
                for (int i = 0; i < NUM_TRIALS; i += 1) {
                    ...
                    totalAddFirstTime += (double) addTime / 1_000_000_000;
                    ...
                    addTime = System.nanoTime() - addStart;
                    // Convert from nanoseconds to seconds and add to total time
                    totalAddTime += (double) addTime / 1_000_000_000;

                    // Measure the time to remove first integers
                    long removeStart = System.nanoTime();
                    for (int j = 0; j < size; j += 1) {
                        deque.removeFirst();
                    }
                    long removeTime = System.nanoTime() - removeStart;
                    // Convert from nanoseconds to seconds and remove to total time
                    totalRemoveFirstTime += (double) removeTime / 1_000_000_000;

                    // Measure the time to remove last integers
                    ...
                    // Convert from nanoseconds to seconds and remove to total time
                    totalRemoveTime += (double) removeTime / 1_000_000_000;
                ...
                System.out.printf("%.10f", totalAddFirstTime / NUM_TRIALS);
                System.out.print(',');
                System.out.printf("%.10f", totalAddTime / NUM_TRIALS);
                System.out.print(',');
                System.out.printf("%.10f", totalRemoveFirstTime / NUM_TRIALS);
                System.out.print(',');
                System.out.printf("%.10f", totalRemoveTime / NUM_TRIALS);
                System.out.print(',');
